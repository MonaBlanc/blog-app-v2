    ...
    <nav ref={navRef} className="h-screen w-60 shadow-sm bg-secondary-light dark:bg-secondary-dark flex flex-col justify-between">
      <div>
        ...
        <Link href="/admin" className="flex items-center space-x-2 p-3">
          ...
          {visible && <span className="text-highlight-light dark:text-highlight-dark text-xl font-semibold">
            Admin
          </span>}
        </Link>
        {/** Nav Items */}
            ...
            {navItems.map((item) => {
              return (
                <li className="flex items-center space-x-2 p-3 text-highlight-light dark:text-highlight-dark hover:scale-[0.98] transition">
                  ...
                  />
                  {visible && <Link key={item.href} href={item.href}>
                    {item.label}
                  </Link>}
                </li>
              );
      ...
      {/** nav toggler (button) */}
      <button onClick={updateNavState} className="p-3 text-highlight-light dark:text-highlight-dark hover:scale-[0.98] transition self-end">
        {visible? <RiMenuFoldLine size={25} />: <RiMenuUnfoldLine size={25} />}
      </button>
    </nav>
  );
};
